...
const Patient = {
  ...
  async getPatient(id, transaction = null) {
    const query =
      "  SELECT Patient.PatientID,Patient.Weight,Patient.BloodPressure,Patient.Height,Patient.Allergies,Patient.AliveStatus,Patient.BloodType,Patient.MedicalHistory,Patient.HospitalID, Person.*,PatientSymptoms.SymptomName,PatientSymptoms.SymptomID,PatientSymptoms.DateRecorded,PatientSymptoms.[Description],Users.Username,PrescrIption.Diagnosis,Prescription.TreatmentID,Prescription.PrescriptionID,Prescription.Advice,Prescription.CaseType,Prescription.DateStarted,Appointment.AppointmentDate,Appointment.DoctorID FROM Patient JOIN Person ON Person.UserID= Patient.PatientID JOIN PatientSymptoms ON PatientSymptoms.PatientID = Patient.PatientID JOIN Prescription ON Prescription.SymptomID = PatientSymptoms.SymptomID JOIN Appointment ON Appointment.PatientID = Patient.PatientID  JOIN Users ON Users.UserID = Patient.PatientID WHERE Patient.PatientID =@id  ";
    const parameters = [{ name: "id", type: sql.Int, value: id }];
    const result = await executeQuery(query, parameters, transaction);
    return result;
  },
