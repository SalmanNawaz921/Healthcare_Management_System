...
import { Calendar, Badge, Tooltip, Button, message } from "antd";
import { useDeleteAppointmentMutation } from "@/redux/services/api/appointmentApi";
...
const AppointmentsCalendar = ({ appointments }) => {
  ...
  const [
    deleteAppointment,
    {
      ...
      error: appointmentDeletionError,
    },
  ...
  const handleCancelAppointment = async(appointment) => {
    ...
    if (appointment?.["AppointmentStatus"] === "Done") {
      message.info("Appointment is already done");
      return;
    } else if (appointment?.["AppointmentStatus"] === "Pending") {
      try{
        const result =  await deleteAppointment(appointment.AppointmentID,localStorage.getItem("Patienttoken"));
        if (result) {
          message.success("Appointment Cancelled Successfully");
          return result;
        }
      }catch(error){
        message.error("Error: Appointment cannot be cancelled");
      }
    }
  };
