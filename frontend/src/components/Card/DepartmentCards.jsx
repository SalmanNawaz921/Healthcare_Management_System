...
import { departmentCards } from "@/constants/constants";

const DepartmentCards = () => {
  const authToken = localStorage.getItem("Hospital Admintoken");
  const { data } = useDepartmentCrud();
  const { data: allEarnings } = useGetDepartmentEarningsQuery(authToken);
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-8">
      {departmentCards?.map((department, i) => (
        <StyledCard
          key={i}
